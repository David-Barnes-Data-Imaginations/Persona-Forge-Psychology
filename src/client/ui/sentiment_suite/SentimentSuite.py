...
def dashboard_all_models():
    ...
    for model_name, result_data in analysis_store.results.items():
        ...
        # Explicit model classification by output structure
        if isinstance(result_data[0], dict) and "utterance" in result_data[0]:
            kind = "utterance"
        elif isinstance(result_data[0], (dict, SentimentSummary)) and "emotion" in result_data[0]:
            kind = "summary"
        else:
            kind = "summary"  # fallback
...
@app.post("/analyze-modernbert")
def analyze_modernbert(file: UploadFile = File(...)):
    ...
    for _, row in df.iterrows():
        ...
        try:
            outputs = classifier(utt)[0]  # Top 5 emotion predictions
            top_emotion = outputs[0]["label"]
            valence, arousal = modernbert_va_map.get(top_emotion, (0.0, 0.0))
            record = {
                "utterance": utt,
                "model": "modernbert-va",
    ...
    analysis_store.results['modernbert-va'] = results
    analysis_store.timestamp = datetime.now()
    return results
# <---- my attempt at adding the old logic but switching to the hermes models
            ...
            <body>
                ...
                <div class="upload-form">
                    <form id="uploadForm" enctype="multipart/form-data">
                        <select name="method" class="method-select">
                            <option value="modernbert">ModernBERT Summary</option>
                            <option value="modernbert-va">ModernBERT Analysis</option>
                        </select>
                        <br>
                ...
                <script>
                    document.getElementById('uploadForm').onsubmit = async (e) => {
                        ...
                        switch(method) {
                            case 'modernbert-va': endpoint = 'analyze-modernbert'; break;
